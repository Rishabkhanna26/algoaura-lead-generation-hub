      ...
      <div className="container-narrow">
        <div className="text-center mb-14 animate-fade-up">
          <span className="terminal-text text-primary text-sm font-semibold uppercase tracking-wider flex items-center justify-center gap-2">
            <Terminal size={16} /> Client System Logs
          </span>
          <h2 className="text-4xl md:text-5xl font-heading font-bold mt-3">
            Verified <span className="gradient-text">Output Logs</span>
          </h2>
        </div>
          ...
          {logs.map((log, index) => (
            ...
            >
              <div className="flex items-center gap-2 mb-3">
                <span className="terminal-text text-[10px] text-primary/60">[{log.id}]</span>
              ...
              <div className="flex items-center gap-2 border-t border-border/50 pt-3">
                <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center">
                  <span className="text-xs font-bold text-primary">{log.name[0]}</span>
                </div>
                <div>
                  <p className="font-heading font-semibold text-xs">{log.name}</p>
                  <p className="text-muted-foreground text-[10px] terminal-text">{log.role}</p>
                </div>
              </div>
            </div>
          ))}
